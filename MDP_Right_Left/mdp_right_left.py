...
'''
# Value Iteration
Converge_Value, ValueIter_Total_Value , ValueIter_Episode = value_evaluation(Reward, Value, \
    TransitionProbaility, GAMMA, DELTA, STATES, ACTIONS)
Value_Final_Policy = policy_determination(Reward, Converge_Value, TransitionProbaility, GAMMA, Policy, \
    Terminal, STATES, ACTIONS, ACTION_LIST)
...
x_axis_length = math.ceil(max(ValueIter_Episode, PolicyIter_Episode, ModPolicyIter_Episode) * 1.1)
...
plt.title('Markov Decision Process (Right/Left)')
plt.legend()
plt.show()
